...
* and $c(I_i),c'(I_i),s(I_i)$ are the per-gadget completeness/soundness values (true-clause ⇒ at least $c$, false-clause ⇒ at most $s$; $c'$ is the “completeness with globals free”). This “sum of per-clause contributions” is the standard additivity in gadget reductions for MAX-CUT / MAX-$k$-CUT. ([Luca Trevisan][1])

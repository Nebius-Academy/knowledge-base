...
Of course, "open-ended chatting" sounds very vague, so I'll try to explain what the authors seek for by showing which benchmarks are especially important for them:

WB (WildBench)

    What it is: A benchmark consisting of 1,024 diverse and challenging prompts sourced from real users in the wild. Many of these prompts involve multiple conversational turns.
    How it evaluates: WildBench uses a detailed, manually-checked rubric to score responses. A human expert (or a model guided by one) checks if the response meets specific quality criteria. The score is given relative to a baseline response from GPT-4, ranging from -100 (much worse) to +100 (much better).
    What it really evaluates: The ability to produce high-quality, nuanced responses that adhere to the complex and often implicit expectations of real users. Because it uses a detailed rubric, it's good at measuring specific capabilities like following constraints, providing detailed explanations, and maintaining a helpful tone.

AE2 (AlpacaEval 2)

    What it is: A benchmark with 805 user prompts designed to test general instruction-following and conversational skill.
    How it evaluates: This benchmark uses a powerful AI judge (in this case, GPT-4o). The judge compares the model's response to a reference response (from a strong model like GPT-4-Turbo) and decides which one it prefers. The final score is the "win rate" of the model being tested. The paper uses a "length-controlled" version to prevent models from winning just by being more verbose.
    What it really evaluates: Overall human preference and perceived quality. It measures how helpful, well-written, and engaging a model's response is from the perspective of a powerful AI proxy for human judgment. It's a more holistic measure than WildBench's rubric.

AH2 (ArenaHardV2)

    What it is: A benchmark of 500 particularly challenging and difficult real-world queries, collected from the Chatbot Arena platform. As the name implies, these are not average user questions. The paper notes it has a high proportion of math and coding questions.
    How it evaluates: Similar to AlpacaEval, it uses a powerful AI judge (GPT-4.1) to determine preference in a head-to-head comparison. It includes "style control" to reduce potential biases in the judging process.
    What it really evaluates: The model's ability to handle the "hard tail" of user requests. This is a stress test of a model's reasoning, problem-solving, and knowledge on complex topics where simpler models often fail. A high score here indicates very strong and robust reasoning capabilities.

AvgChat

    What it is: This isn't a benchmark itself. It is simply the mathematical average of the scores from the three benchmarks above (WB, AE2, and AH2).
    What it evaluates: It provides a single, easy-to-compare number that summarizes the model's overall performance across the different facets of conversational ability tested by the individual benchmarks. It's a composite score for general chat skill.

...
**Paperwatch ****28****.****0****5****.202****5**** by Stanislav Fedotov (Nebius Academy)**
# New Models, Services, and Frameworks
## Claude 4
[https://www.anthropic.com/news/claude-4]({'url': 'https://www.anthropic.com/news/claude-4', 'bold': False, 'italic': False})
...
## VEO-3 by Google
[https://deepmind.google/models/veo/]({'url': 'https://deepmind.google/models/veo/', 'bold': False, 'italic': False})
[https://blog.google/technology/ai/google-flow-veo-ai-filmmaking-tool/]({'url': 'https://blog.google/technology/ai/google-flow-veo-ai-filmmaking-tool/', 'bold': False, 'italic': False})
...
## OpenAI Codex
[https://openai.com/codex/]({'url': 'https://openai.com/codex/', 'bold': False, 'italic': False})

![]({{ site.baseurl }}/assets/images/paperwatch-28.05.2025/4-image-4.png){: .responsive-image style="--img-desktop:75%; --img-mobile:90%;"}
...
[https://blog.google/technology/google-deepmind/gemini-diffusion/]({'url': 'https://blog.google/technology/google-deepmind/gemini-diffusion/', 'bold': False, 'italic': False})
From time to time, researchers challenge transformer architecture in pursuit of something better – which usually means something faster. Once, State Space Models (including the famous Mamba) were a candidate transformer killers. Now, there are some experiments around [Diffusion LLMs]({'url': 'https://arxiv.org/pdf/2502.09992', 'bold': True, 'italic': False}). Now, Google shipped such a model of their own.
...
[https://mistral.ai/news/devstral]({'url': 'https://mistral.ai/news/devstral', 'bold': False, 'italic': False})
An open source coding LLM from Mistral and [All Hands AI]({'url': 'https://www.all-hands.dev/', 'bold': False, 'italic': False}) 🙌, which seems to outperform other open source coding LLMs, even (much) larger ones.
...
[https://arxiv.org/pdf/2505.13898]({'url': 'https://arxiv.org/pdf/2505.13898', 'bold': False, 'italic': False})
...
- [Transformer layers as painters]({'url': 'https://arxiv.org/pdf/2407.09298', 'bold': False, 'italic': False}), introducing the idea of middle layers having some subtle specializations, like a painter receiving the canvas from the painter below and deciding whether to add a few strokes to the painting or just pass it along to the painter above her (using the residual connections).
...
- [Logit lens]({'url': 'https://www.lesswrong.com/posts/AcKRB8wDpdaN6v6ru/interpreting-gpt-the-logit-lens', 'bold': False, 'italic': False}), which is a nice tool for peeking into the transformer internals. How it works: after each transformer layer, we may apply the unembedding layer and look at the predicted tokens. Curious ways in which these predictions change from the bottom to the top layers might sometimes bring curious insights.
This paper also hints that some layers are slacking, but it gives this another perspective.
A transformer layer has two main components – self-attention and FFN (with additional normalization here and there), and each of them is added alongside a **residual connection**, like, having the initial hidden state $[hl]$:
$[hl =hl+ SelfAttentionlNormhl]$
$[hl+1=hl+MLP(Normhl)]$

![]({{ site.baseurl }}/assets/images/paperwatch-28.05.2025/10-image-10.png){: .responsive-image style="--img-desktop:75%; --img-mobile:90%;"}

This might indeed allow a transformer block not to do contribute much and to just leave $[hl]$ mostly as it was. And a natural way of checking this is comparing magnitudes of the initial $[hl]$ and of the things being added to it (“*layer contributions*”) while producing $[hl+1]$:
...
The second plot is quite peculiar. The authors measured cosine similarity between $[hl]$ and the sublayer contributions – and it seems to follow a distinct patterns, which the authors interpret in the following way:
...
- Without intervention, obtaining downstream representations $[hs]$ ($[s>t)]$
- Switching off the layer $[ft]$ in $[ht+1=ht+ft]$ and obtaining modified $[hs]$

![]({{ site.baseurl }}/assets/images/paperwatch-28.05.2025/12-image-12.png){: .responsive-image style="--img-desktop:75%; --img-mobile:90%;"}

Then they compute the relative contribution of $[ft]$:
$[hs+1-hs- hs+1- hs2hs+1-hs2]$
...
Interestingly though, a [Universal Transformer]({'url': 'https://arxiv.org/pdf/2405.16039', 'bold': False, 'italic': False}) model trained by the authors exhibit increasing “useful depth” with increasing problem complexity. It would be interesting to see further developments in this topic.
# Beyond Semantics: The Unreasonable Effectiveness Of Reasonless Intermediate Tokens
[https://arxiv.org/pdf/2505.13775]({'url': 'https://arxiv.org/pdf/2505.13775', 'bold': False, 'italic': False})
...
The authors generate a number of mazes with Wilson’s Algorithm (see, for example, [this post]({'url': 'https://medium.com/@batbat.senturk/the-ultimate-unbiased-maze-generation-technique-you-need-to-see-46123d5fec76', 'bold': False, 'italic': False}) for an explanation and some visualizations, or check the paper itself), run A* in them and train an LLM in three ways:
...
Now, isn’t this another hint that true reasoning happens deep inside an LLM and might be not connected directly to the “though-like” outputs? (Another similarly powerful hint is [Let’s Think Dot By Dot]({'url': 'https://arxiv.org/pdf/2404.15758', 'bold': False, 'italic': False}), of course.)
# When Thinking Fails: The Pitfalls of Reasoning for Instruction-Following in LLMs
[https://arxiv.org/pdf/2505.11423]({'url': 'https://arxiv.org/pdf/2505.11423', 'bold': False, 'italic': False})
LLM Reasoning is often beneficial, but sometimes not. For example, the [To CoT or not to CoT]({'url': 'https://arxiv.org/pdf/2409.12183', 'bold': False, 'italic': False}) paper established that Chain-of-Thought reasoning doesn’t help (and sometimes can even hurt) in knowledge-related questions. Now, what about long reasoning? Can it hurt?
...
In my opinion, it’s a bit strange to expect from a long reasoner to abide to such down-to-earth constraints; they are needed for different scenarios – and if one needs an answer in $[≤100]$ words or an answer in a very specific format, that can be established with post-processing. It’s not surprising that in the end the authors observe that the best way of countering this problem is **Classifier-Selective Reasoning** – using a classifier (another LLM) to decide whether long reasoning is needed, before actually asking the main LLM to produce the solution.
...
[https://arxiv.org/pdf/2505.10475]({'url': 'https://arxiv.org/pdf/2505.10475', 'bold': False, 'italic': False})
...
Parallel Scaling, as they describe it, resembles both Bagging and Boosting at the same time. The idea is, having a single base model $[fθ]$ and an input $[x]$:
- Apply $[P]$ learnt transformations to $[x]$, transforming it into $[x1,x2,…,xP]$
- Run $[x1,x2,…,xP]$ in parallel through the model $[fθ]$ (parallel execution here makes things more efficient in comparison with sequential long reasoning)
...
$[w1fθx1+w2fθx2+…+wPfθ(xP)]$
with learnable $[w1,…wP]$.
...
They start with the well-known [Chinchilla Scaling Law]({'url': 'https://arxiv.org/pdf/2203.15556', 'bold': False, 'italic': False}), “forget” about the dataset-size-related summand and mathematically infer the law for the parallel stream configuration:
Then they fit the parameters $[A]$, $[α]$ and $[E]$ with a proxy $[klogP + 1]$ for the diversity.
...
[https://arxiv.org/pdf/2505.03336]({'url': 'https://arxiv.org/pdf/2505.03336', 'bold': False, 'italic': False})
...
![]({{ site.baseurl }}/assets/images/paperwatch-28.05.2025/23-image-23.png){: .responsive-image style="--img-desktop:75%; --img-mobile:90%;"}
...
Then,  after *<SOI>*, the LLM will only be able to choose between *Fall* and *Call*; if these tokens get probabilities 0.1 and 0.2, they will be renormalized as $[0.10.1+0.2=13]$ and $[0.20.1+0.2=23]$, and either *Fall* or *Call* will be sampled according to these new probabilities. Then, if *Fall* is chosen, one of the tokens $[out]$ and $[down]$ will be predicted based on the probabilities predicted by the LLM from “*…<SOI> Fall*”
...
[https://arxiv.org/pdf/2505.15809]({'url': 'https://arxiv.org/pdf/2505.15809', 'bold': False, 'italic': False})
...
In the [original Diffusion LLM paper]({'url': 'https://arxiv.org/pdf/2502.09992', 'bold': False, 'italic': False}), this was done with masking/mask prediction:
The training had two stages:

![]({{ site.baseurl }}/assets/images/paperwatch-28.05.2025/27-image-27.png){: .responsive-image style="--img-desktop:75%; --img-mobile:90%;"}
...
![]({{ site.baseurl }}/assets/images/paperwatch-28.05.2025/31-image-31.png){: .responsive-image style="--img-desktop:75%; --img-mobile:90%;"}
  ...
  - **Multimodal Reasoning Rewards**: For math-related (geometry) tasks such as [GeoQA]({'url': 'https://arxiv.org/pdf/2105.14517', 'bold': False, 'italic': False}) and [CLEVR]({'url': 'https://arxiv.org/pdf/1612.06890', 'bold': False, 'italic': False}), it’s the same correctness and format. In addition, for caption-based tasks, **CLIP Reward** is used:                             $[0.1 · CLIP(image, text)]$.
...
[https://arxiv.org/pdf/2505.10320]({'url': 'https://arxiv.org/pdf/2505.10320', 'bold': False, 'italic': False})
...
![]({{ site.baseurl }}/assets/images/paperwatch-28.05.2025/33-image-33.png){: .responsive-image style="--img-desktop:75%; --img-mobile:90%;"}

- [Preference Proxy Evaluations (PPE)]({'url': 'https://openreview.net/pdf?id=cbttLtO94Q', 'bold': True, 'italic': False}) – a large-scale benchmark that links reward models to real-world human preference performance. It consists of two subsets:
...
- [JudgeBench]({'url': 'https://arxiv.org/pdf/2410.12784', 'bold': True, 'italic': False}) (350 preference pairs; subset with responses generated by GPT-4o) - challenging response pairs that span knowledge, reasoning, math, and coding categories; the comparison in these pairs was made with priority for factual and logical correctness as opposed to stylistic alignment etc.
- [RM-Bench]({'url': 'https://openreview.net/pdf?id=QEHrmQPBdd', 'bold': True, 'italic': False}) (4K samples) a dataset that assesses the robustness of reward models based on their sensitivity and resistance to subtle content differences and style biases.

![]({{ site.baseurl }}/assets/images/paperwatch-28.05.2025/34-image-34.png){: .responsive-image style="--img-desktop:75%; --img-mobile:90%;"}

- [FollowBenchEval]({'url': 'https://arxiv.org/pdf/2501.18099', 'bold': True, 'italic': False}) (205 preference pairs) tests reward models for their ability to validate multi-level constraints in LLM responses (e.g., “Write a one sentence summary (less than 15 words) for the following dialogue. The summary must contain the word ‘stuff’...”).
- [RewardBench]({'url': 'https://arxiv.org/pdf/2403.13787', 'bold': True, 'italic': False}) (3K samples), similar to JudgeBench, consists of preference pairs from 4 categories of prompts: chat, chat-hard, safety, and reasoning.
**Training data**
For training, the authors use **synthetic data**, with prompts from 17K [WildChat]({'url': 'https://openreview.net/pdf?id=Bl8u7ZRlbM', 'bold': False, 'italic': False}) and 5K [MATH]({'url': 'https://openreview.net/pdf?id=7Bywt2mQsCe', 'bold': False, 'italic': False}) and pairs of accepted/rejected answers obtained as follows:
